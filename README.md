...
适用于 Minecraft 基岩版 Windows 客户端的轻量级结构投影模组（Litematica 风格），基于 LeviLamina client。

LHolo 只在客户端绘制虚拟结构，不向世界写入方块、不修改存档，服务器无需安装任何配套插件。
...
- 水/岩浆贴图投影（原版 terrain atlas，纯客户端显示）
